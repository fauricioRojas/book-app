...
export const Pet: FC<PetProps> = ({ serverPet }) => {
  const [{ id, name, breed, notes, procedures }, setPet] =
    useState<TPet>(serverPet);
  ...
  const handleShowPetsFormInEditMode = () => {
    ...
    showDrawer({
      ...
      children: (
        <PetsForm defaultValues={defaultValues} petId={id} noteId={notes.id} />
      ),
    });
  };
